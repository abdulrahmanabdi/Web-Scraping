...
def extract_content_from_page(file_path):
    ...
    parsed_data['date'] = soup.find('div', {'class': 'date'}).text[:10]

    parsed_data['title'] = soup.find('div', {'class': 'content_mod'}).find('h2').text

    parsed_data['content'] = soup.find('div', {'class': 'content'}) \
                                .find('div', {'class': 'trs_editor_view'}).text
