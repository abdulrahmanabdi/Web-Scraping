...
def download_links_from_index():
    ...
    for pid in range(start_page-1, 10):
        if pid == 0:
            pageURL = "https://www.fmprc.gov.cn/mfa_eng/wjbxw/index.html"
        else:
            pageURL = f"https://www.fmprc.gov.cn/mfa_eng/wjbxw/index_{pid}.html"
        ...
        for item in soup.find("div", {"class": "newsLst_mod"}).find_all("a"):

            #print(item)
            print(item['href'])
            # Save the collected url in the variable "collected_url"

            collected_url = 'https://www.fmprc.gov.cn/mfa_eng/wjbxw' + item['href'][1:]
